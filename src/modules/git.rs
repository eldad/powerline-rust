...
impl<S: GitScheme> Module for Git<S> {
    fn append_segments(&mut self, powerline: &mut Powerline) {
        ...
        let mut add_elem = |count: u32, symbol: &str, fg, bg| match count.cmp(&1) {
        ...
        add_elem(stats.non_staged, "✏️", S::GIT_NOTSTAGED_FG, S::GIT_NOTSTAGED_BG);
        add_elem(stats.untracked, "❓", S::GIT_UNTRACKED_FG, S::GIT_UNTRACKED_BG);
        add_elem(stats.conflicted, "✳️", S::GIT_CONFLICTED_FG, S::GIT_CONFLICTED_BG);
        ...
        if let Some(fetch_head_age) = stats.fetch_head_age {
            if fetch_head_age.as_secs() > 3600 {
                ...
                let fg =
                    if fetch_head_age.as_secs() > 12 * 3600 { S::GIT_FETCH_AGE_FG } else { S::GIT_FETCH_AGE_STALE_FG };

                powerline.add_segment(
                    format!("{}{}", duration_string(&fetch_head_age), symbol),
                    Style::simple(fg, S::GIT_FETCH_AGE_BG),
                );
            }
        }
    }
